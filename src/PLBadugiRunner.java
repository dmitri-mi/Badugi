...
public class PLBadugiRunner {
    ...
    public static final int HANDS_PER_MATCH = 100000;
    // How often to print out the current hand even when silent (-1 means never)
    private static final int SAMPLE_OUTPUT= 245678;
    ...
    public static int playOneHand(int handSize, EfficientDeck deck, PLBadugiPlayer[] players, PrintWriter out, int handsToGo, int currentScore) {
        ...
        if(showdown != 0) {
            message(out, players[showdown > 0 ? 0 : 1].getAgentName() +" won " + totalBets[0] + " chips.");
        }
        else {
            message(out, "Both players brought equal badugi hands to showdown.");
        }
        try { players[0].handComplete(hands[0], hands[1], result); } catch(Exception e) { }
        try { players[1].handComplete(hands[1], hands[0], result); } catch(Exception e) { }
    ...
    public static void playThreeHandTournament() throws IOException {
        PLBadugiPlayer[] players = {
                ...
                new PLBadugi500877176(), new PLBadugi500877176()
        };
        Random rng;
        String seed = "This string is to be used as seed of secure random number generator "+System.currentTimeMillis();
        try { rng = new SecureRandom(seed.getBytes()); }
    ...
    public static void main(String[] args) throws IOException {
        ...
        //         "IlkkaPlayer3", "IlkkaPlayer3", "IlkkaPlayer3", "IlkkaPlayer3", "IlkkaPlayer3", "IlkkaPlayer3",
        // };
        //
